...
def ensure_collection(client: MilvusClient) -> None:
    ...
    schema.add_field("image_path", DataType.VARCHAR, max_length=512)   # ← new
...
def embed_and_upsert(chunks: list[Chunk], doc_id: str) -> None:
    ...
    image_path stored for figure/table chunks that have saved images.
    """
    if not chunks:
    ...
    for chunk, emb in zip(chunks, embeddings):
        rows.append({
            ...
            "type":       chunk.type,
            "doc_id":     doc_id,
            "image_path": chunk.image_path or "",   # ← empty string for non-image chunks
...
if __name__ == "__main__":
    ...
    doctree = ingest_pdf(pdf_path, doc_id=doc_id)   # pass doc_id for figures dir
